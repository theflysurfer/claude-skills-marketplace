...
def define_env(env):
    ...
    @env.macro
    def servers_table_with_actions(category: str = None) -> str:
        """Generate a table of servers with start/stop action buttons."""
        servers_file = config_dir / "servers-registry.json"
        if not servers_file.exists():
            return "*Registre serveurs non trouvé*"
        ...
        lines = [
            ...
            '<thead><tr>',
            '<th>Serveur</th><th>Description</th><th>Port</th><th>Startup</th><th>Actions</th>',
            '</tr></thead>',
            '<tbody>'
        ]
        ...
        for server_id, info in sorted(servers.items()):
            ...
            start_cmd = f"python scripts/server-manager.py start {server_id}"
            stop_cmd = f"python scripts/server-manager.py stop {server_id}"

            lines.append(f'<tr>')
            lines.append(f'<td><code>{server_id}</code></td>')
            lines.append(f'<td>{desc}</td>')
            lines.append(f'<td>{port}</td>')
            lines.append(f'<td>{startup}</td>')
            lines.append(f'<td>')
            lines.append(f'<button class="md-button md-button--primary server-btn" data-server-action="start" data-server="{server_id}" data-copy-command="{start_cmd}">▶ Start</button> ')
            lines.append(f'<button class="md-button server-btn" data-server-action="stop" data-server="{server_id}" data-copy-command="{stop_cmd}">⏹ Stop</button>')
            lines.append(f'</td>')
            lines.append(f'</tr>')

        lines.append('</tbody></table>')
        lines.append('')
        lines.append('<small>💡 Cliquez sur un bouton pour copier la commande</small>')
