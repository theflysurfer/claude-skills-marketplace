...
2. **Listé** dans `registry/sync-config.json` → `scripts_to_sync`
...
│   └── ...                     │   └── ...
└── registry/                    └── settings.json (references scripts)
...
config = json.loads(Path('registry/sync-config.json').read_text())
...
# Éditer registry/sync-config.json:
...
# 6. Commit
git add scripts/mon-nouveau-hook.py registry/sync-config.json
